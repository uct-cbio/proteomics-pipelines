...
~~~~
Configure general parameters. The THREAD_LIMIT multiplied by JVM_Xmx should not be higher than the total RAM available per node, and THREAD_LIMIT should be one less than the total number of available cores. Out of memory issues can be corrected by reducing CHUNKSIZE, assuming the THREAD_LIMIT and JVM_Xmx are correct for the available resources.
~~~~
