    ...
    for strain in strain_peps:
        ...
        for p in strain_exclusive_novel:
            for refp in refps:
                ...
                out = sequtils.pairwise_blast(rec, refp, output)
                header = '***Results for peptide {} in record {}***'.format(p, refp.id) 
                blasted.append(header)
                blasted.append(out.results)
                blasted.append('\nIdentified polymorphism positions: {}'.format(';'.join([str(i) for i in out.differences])))
                overlap = out.feature_overlap(reference_features)
                if overlap != None: 
