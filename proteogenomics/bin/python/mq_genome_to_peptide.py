...
for strain in config.strains:
    ...
    g2p = sequtils.gssp(genome, assembly_name = strain, translation_table=config.translation_table, peptides_list=peptide_sequences, threads=config.threads)
    speps = g2p.peptides
    ...
    speps.to_csv(strainpath + '/' + '{}_mapped_peptides.csv'.format(strain))
