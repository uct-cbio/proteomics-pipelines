...
for strain in config.strains:
    ...
    if not os.path.exists( output +'/{}/WIG'.format(strain)):
        os.mkdir(output + '/{}/WIG'.format(strain))
    ...
    for contig in contig_dict:
        ...
        for strand in contig_dict[contig]:
            ...
            for position in positions:
                ...
                ids.append(tup[0])
                coords = tup[0].split()[1].split('|')[-1].split(')')[1].split(':')
                coords = (int(coords[0]), int(coords[1]))
                for i in range(len(tup[1])):
                    samplecol = intensity_cols[i]
                    sampleval = tup[1][i]
                    if not samplecol in sample_dict:
                        sample_dict[samplecol] = defaultdict(list)
                    if not str(sampleval) == 'nan':
                        for cpos in range(coords[0] , coords[1] + 1):
                            sample_dict[samplecol][cpos].append(sampleval)
