...
function shakemgf {
    ...
    wait
    java $JVM_ARGS -cp ${sample_folder}/sg/SearchGUI-*.jar eu.isas.searchgui.cmd.PathSettingsCLI -temp_folder ${sample_folder}/temp -log ${sample_folder}/log  
    wait
    echo "Started SearchCLI for: "${sample}"..." 
    java $JVM_ARGS -cp ${sample_folder}/sg/SearchGUI-*.jar eu.isas.searchgui.cmd.SearchCLI -spectrum_files ${spectrum_file} -output_folder ${sample_folder} -id_params ${output_folder}"/identification.par" -output_data "${output_data}" -xtandem "${xtandem}" -myrimatch "${myrimatch}" -ms_amanda "${ms_amanda}" -msgf "${msgf}" -omssa "${omssa}" -comet "${comet}" -tide "${tide}" -andromeda "${andromeda}" >> ${sample_folder}/log/${sample_name}.out 2>&1
    wait
    echo "Started PeptideShakerCLI for: "${sample}"..." 
    java $JVM_ARGS -cp ${sample_folder}/ps/PeptideShaker-*.jar eu.isas.peptideshaker.cmd.PeptideShakerCLI -experiment ${experiment} -sample ${sample} -replicate  ${replicate} -identification_files  ${sample_folder}"/searchgui_out.zip"  -spectrum_files  ${spectrum_file} -id_params  ${output_folder}"/identification.par" -out ${sample_folder}"/"${sample_name}".cpsx" >> ${sample_folder}/log/${sample_name}.out 2>&1
    wait
    echo "Started FollowUpCLI recalibration for: "${sample}"..." 
    java $JVM_ARGS -cp ${sample_folder}/ps/PeptideShaker-*.jar eu.isas.peptideshaker.cmd.FollowUpCLI -in ${sample_folder}"/"${sample_name}".cpsx" -recalibration_folder ${sample_folder}/recalibrated >> ${sample_folder}/log/${sample_name}.out 2>&1
    wait
    echo "Started recalibrated sample SearchCLI for: "${sample}"..." 
    java $JVM_ARGS -cp ${sample_folder}/sg/SearchGUI-*.jar eu.isas.searchgui.cmd.SearchCLI -spectrum_files ${sample_folder}/recalibrated/${sample_name}_recalibrated.mgf  -output_folder ${sample_folder}/recalibrated -id_params ${output_folder}"/identification.par" -output_data "${output_data}" -xtandem "${xtandem}" -myrimatch "${myrimatch}" -ms_amanda "${ms_amanda}" -msgf "${msgf}" -omssa "${omssa}" -comet "${comet}" -tide "${tide}" -andromeda "${andromeda}" >> ${sample_folder}/log/${sample_name}.out 2>&1 
    wait
    echo "Started recalibrated sample PeptideShakerCLI for: "${sample}"..."  
    java $JVM_ARGS -cp ${sample_folder}/ps/PeptideShaker-*.jar eu.isas.peptideshaker.cmd.PeptideShakerCLI -experiment ${experiment} -sample ${sample} -replicate ${replicate} -identification_files ${sample_folder}"/recalibrated/searchgui_out.zip" -spectrum_files ${sample_folder}/recalibrated/${sample_name}_recalibrated.mgf -id_params ${output_folder}"/identification.par" -out ${sample_folder}/recalibrated/${sample_name}".cpsx"  >> ${sample_folder}/log/${sample_name}.out 2>&1
    wait
    echo "Started recalibrated sample MzidCLI for: "${sample}"..." 
    java $JVM_ARGS -cp ${sample_folder}/ps/PeptideShaker-*.jar eu.isas.peptideshaker.cmd.MzidCLI -in ${sample_folder}/recalibrated/${sample_name}".cpsx" -output_file ${mzid_folder}"/"${sample_name}".mzid" -contact_first_name "${contact_first_name}" -contact_last_name "${contact_last_name}" -contact_email "${contact_email}" -contact_address "${contact_address}" -organization_name "${organization_name}" -organization_email "${organization_email}" -organization_address "${organization_address}" -contact_url "${contact_url}" -organization_url "${organization_url}" >> ${sample_folder}/log/${sample_name}.out 2>&1
    wait
    echo "Started recalibrated sample FollowUpCLI to export unvalidated spectra for: "${sample}"..."  
    java $JVM_ARGS -cp ${sample_folder}/ps/PeptideShaker-*.jar eu.isas.peptideshaker.cmd.FollowUpCLI -in ${sample_folder}/recalibrated/${sample_name}.cpsx -spectrum_folder ${unvalidated_folder} -psm_type ${psm_type}>> ${sample_folder}/log/${sample_name}.out 2>&1
    wait
...
(
  ...
  for spectrum_file in ${spectrum_files}/*.mgf; do
    replicate=${count}
    (shakemgf ${spectrum_file} ${experiment_name} ${replicate} || kill 0) &
    let count+=1
  done 
