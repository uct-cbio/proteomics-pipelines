...
function shakemgf {
    ...
    java -cp ${temp_folder}/sg/SearchGUI-*.jar eu.isas.searchgui.cmd.SearchCLI -spectrum_files ${spectrum_file} -output_folder ${sample_folder} -id_params ${output_folder}"/identification.par" -output_data "${output_data}" -xtandem "${xtandem}" -myrimatch "${myrimatch}" -ms_amanda "${ms_amanda}" -msgf "${msgf}" -omssa "${omssa}" -comet "${comet}" -tide "${tide}" -andromeda "${andromeda}" & 
    wait

    java -cp ${temp_folder}/ps/PeptideShaker-*.jar eu.isas.peptideshaker.cmd.PeptideShakerCLI -experiment ${experiment} -sample ${sample} -replicate  ${replicate} -identification_files  ${sample_folder}"/searchgui_out.zip"  -spectrum_files  ${spectrum_file} -id_params  ${output_folder}"/identification.par" -out ${sample_folder}"/"${sample_name}".cpsx" & 
    wait

    java -cp ${temp_folder}/ps/PeptideShaker-*.jar eu.isas.peptideshaker.cmd.MzidCLI -in ${sample_folder}"/"${sample_name}".cpsx" -output_file ${mzid_folder}"/"${sample_name}".mzid" -contact_first_name "${contact_first_name}" -contact_last_name "${contact_last_name}" -contact_email "${contact_email}" -contact_address "${contact_address}" -organization_name "${organization_name}" -organization_email "${organization_email}" -organization_address "${organization_address}" -contact_url "${contact_url}" -organization_url "${organization_url}" & 
    wait

    java -cp ${temp_folder}/ps/PeptideShaker-*.jar eu.isas.peptideshaker.cmd.FollowUpCLI -in ${sample_folder}"/"${sample_name}".cpsx" -spectrum_folder ${unvalidated_folder} -psm_type ${psm_type} & 
    wait
}
...
MSnIDshake.R -i ${output_folder}/mzIdentMLs/ -v ${MSnID_FDR_value} -l ${MSnID_FDR_level}
