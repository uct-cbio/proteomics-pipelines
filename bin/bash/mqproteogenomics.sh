...
if [ ! -d $outpath/mapping ] ; then
    mq_peptide_to_referencedb.py $config $outpath || rm -rf $outpath/mapping ; exit 1
fi
...
if [ ! -f $outpath/fasta/nr_translated_pg_orfs.fasta.gff3 ] ; then
    ips.sh $outpath/fasta/nr_translated_pg_orfs.fasta $outpath/fasta $python2ve || rm -rf $outpath/fasta/nr_translated_pg_orfs.fasta.gff3 ; exit 1
fi
