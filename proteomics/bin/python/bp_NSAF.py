...
#   org = val.split('|')[2].split()[0].split('_')[1]
#   return org
...
#data = data[data['SpectralCounts'] > 1]
...
data['ProteinProb'] = data['Summed NSAF'] / data['Summed NSAF'].sum()
...
data = data.sort_values( by='ProteinProb', ascending = False )
...
w= open(prefix +'_100perc_msms_mapped.fasta','w')
w.write('\n'.join(records))
w.close()
...
ss = data_filt.groupby(data_filt.Organism).agg({"Summed NSAF": sum})

n = ss.sort_values('Summed NSAF', ascending=False)

n = n.reset_index()
print(n.head())

n['OrganismProb'] = n['Summed NSAF'] / n['Summed NSAF'].sum()
n = n.set_index('Organism')
org_series = pd.Series(n.OrganismProb)
org_dict = org_series.to_dict()

data['OrganismProb'] = data['Organism'].map(org_dict)

data['CombinedProb'] = data['OrganismProb'] * data['ProteinProb']

data = data.sort_values( by='CombinedProb', ascending = False )
...
data['CumulativeMSMSCount'] = pd.Series(all_cum)
data['CumulativeMSMSPercentage'] = data['CumulativeMSMSCount'] / msms_set_len * 100.0
prefix = "organism_" + sys.argv[2].split('.csv')[0] 
data.to_csv("organism_" + sys.argv[2])
data_filt = data.drop_duplicates('CumulativeMSMSPercentage', keep='first')
filt_prefix=prefix + '_compacted'
data_filt.to_csv(filt_prefix +'.csv')
data_filt_recs = data_filt['Records'].tolist()
